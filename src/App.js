...
function App() {
  ...
  return (
    <div className={finalDisplayTheme === 'Light' ? 'theme-light overflow-x-hidden' : 'theme-dark overflow-x-hidden'}>
      ...
      <div className="bg-white w-96 h-full mt-2 mx-10 p-5 rounded-xl bg-opacity-50">
        <div className="flex">
        ...
        {Array.isArray(tasks.todo) && tasks.todo.length > 0 ? (
          tasks.todo.map((task) => (
            ...
            <div className="text-center text-sm">-x-x-x-</div>
        )}
        <Main onAdd={() => setShowAddTask(!showAddTask)} showAdd={showAddTask} />
      </div>
      <div className="bg-white w-96 h-full mt-2 p-5 rounded-xl bg-opacity-50">
        <div className="flex">
      ...
      <div className=" justify-center md:block lg:hidden mt-24">       
      <div className="bg-white mx-auto w-96 h-full mb-10 p-5 rounded-xl bg-opacity-50">
        <div className="flex">
        <label className="text-xl text-center mr-3 font-bold mr-10">To Do</label>
        <div className="bg-white bg-opacity-50 rounded-full text-xl ml-52 px-2">{tasks.todo.length}</div>
        </div>
        {Array.isArray(tasks.todo) && tasks.todo.length > 0 ? (
          tasks.todo.map((task) => (
              <Tasks sectionTasks={[task]} key={task.id} task={[task]} onDelete={deleteTask} onToggle={reminderTask} />
      ...
      <div className="bg-white mx-auto w-96 h-full mb-10 p-5 rounded-xl bg-opacity-50">
        <div className="flex">
        <label className="text-xl text-center mr-5 font-bold mr-16">In Progress</label>
        <div className="bg-white bg-opacity-50 rounded-full text-xl ml-36 px-2">{tasks.progress.length}</div>
        </div>
        {Array.isArray(tasks.progress) && tasks.progress.length > 0 ? (
          tasks.progress.map((task) => (
              <Tasks sectionTasks={[task]} key={task.id} tasks={[task]} onDelete={deleteTask} onToggle={reminderTask} />
      ...
      <div className="bg-white mx-auto w-96 h-full mb-10 p-5 rounded-xl bg-opacity-50">
        <div className="flex">
        <label className="text-xl text-center mr-5 font-bold mr-10">Done</label>
        <div className="bg-white bg-opacity-50 rounded-full text-xl ml-52 px-2">{tasks.done.length}</div>
        </div>
        {Array.isArray(tasks.done) && tasks.done.length > 0 ? (
          tasks.done.map((task) => (
              <Tasks sectionTasks={[task]} key={task.id} tasks={[task]} onDelete={deleteTask} onToggle={reminderTask} />
